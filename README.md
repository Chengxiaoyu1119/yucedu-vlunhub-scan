...
最新 Windows 发行版：[Releases](https://github.com/Chengxiaoyu1119/yucedu-vlunhub-scan/releases/latest)
...
```bash
python -m scanner_app.cli.public --targets TARGET --port-start 8000 --port-end 8099
```
...
| 构建与发布 | `scripts/` | Windows 构建脚本和 PyInstaller 配置 | 只放构建配置，不放运行代码和构建结果 |
